...
namespace TravelPortal.web.Controllers
{
    ...
    public class AccountController : BaseController
    {
        ...
        public ActionResult Login(string returnUrl)
        {
            ...
            if (CurrentUrl.ToLower().Equals(RouteConfig.AdminLogin.ToLower()))
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    var objUser = _userService.GetUserByEmail(model.Email);
                    if (CurrentUrl.ToLower().Equals(RouteConfig.AdminLogin.ToLower()))
                    {
                        if (objUser.RoleId != 1)
                        {
                        ...
                        SessionHelper.AdminUsrno = objUser.Usrno;
                    ...
                    SessionHelper.Usrno = objUser.Usrno;
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                if (model.PageIndex == 1)
                {
                    ...
                    string otp = rnd.Next(100000, 999999).ToString(CultureInfo.InvariantCulture);
                    // Send OTP Email
                ...
                {
                    if (model.OTP != SessionHelper.OTP)
                    {
                ...
                {                    
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        _userService.AddEdit(models);
                        UserManager.AddToRole(user.Id, RolesType.Agent);
                        ViewBag.IsSuccess = 1;
                        //return RedirectToAction("Login", "Account");
                        SessionHelper.OTP = null;
                        model.PageIndex = 4;
                        return View(model);
                    }
                    AddErrors(result);
        ...
        {
            AuthenticationManager.SignOut(DefaultAuthenticationTypes.ApplicationCookie);
            return Redirect(RouteConfig.AdminLogin.ToPrefixApplicationURL());
        }
